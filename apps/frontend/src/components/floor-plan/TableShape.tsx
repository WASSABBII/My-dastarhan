...
export default function TableShape({ table, status, selected, onClick }: TableShapeProps) {
  ...
  const cursor = status === 'busy' ? 'not-allowed' : 'pointer'

  const shape = () => {
    if (table.shape === 'round') return <circle cx="30" cy="30" r="28" fill={fill} stroke="#fff" strokeWidth="2" />
    if (table.shape === 'square') return <rect width="58" height="58" rx="6" fill={fill} stroke="#fff" strokeWidth="2" />
    return <rect width="88" height="48" rx="6" fill={fill} stroke="#fff" strokeWidth="2" />
  }
  ...
  return (
    <g transform={`translate(${x}, ${y})`} onClick={onClick} style={{ cursor }}>
      {shape()}
      <text
        x={textX} y={textY}
